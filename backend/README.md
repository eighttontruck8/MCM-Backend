...
직원은 로그인 화면의 `직원용 로그인`에서 진입한다. `POST /api/v1/auth/staff/signup`은 이름, 이메일, 4자 이상의 비밀번호, 매장 코드와 4자 이상의 직원 가입 코드를 검증한 뒤 `STAFF` 계정을 생성한다. 직원 가입 기능을 사용하려면 API 환경변수 `M_JOURNEY_STAFF_SIGNUP_CODE`를 설정해야 하며, 이 값은 프론트 코드나 저장소에 기록하지 않는다. 가입 완료 후 직원은 소속 매장의 대기열 화면으로 이동한다.
