...
실제 매장에서는 QR 스캔이 기본이다. 로그인한 고객이 홈 상단의 `체크인` 버튼을 누르면 `GET /api/v1/stores`의 활성 매장을 브라우저 위치 기준으로 정렬하고, 선택한 매장에 `POST /api/v1/check-ins/store`로 직접 체크인한다. 매장 확인 팝업의 `예`는 `restart_active=true`로 전송하며, 같은 매장에 종료되지 않은 이전 방문이 있으면 종료한 뒤 새 방문을 시작한다. 위치 권한을 거부해도 기본 매장 순서로 이용할 수 있다. `/api/v1/check-ins/demo`는 기존 시연 호환용으로 유지하며 QR 토큰은 프론트 번들에 포함하지 않는다.
