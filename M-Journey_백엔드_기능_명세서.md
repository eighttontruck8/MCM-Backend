...
M-Journey는 고객의 온라인 구매·조회·관심 데이터를 NFC 기반 오프라인 체크인과 연결해 다음 두 경험을 제공한다.
...
- 프론트: React + Vite + JavaScript. 로그인, NFC 로딩, 체크인 완료, 쇼핑 방식, 동의/방문 목적, 직원 배정, 룩북 등의 화면이 구현되어 있으나 실제 API 호출과 상태 저장은 아직 없다.
...
1. NFC에는 `https://{front-host}/?tag={opaque_tag_token}` 형태의 URL을 기록한다.
2. 고객이 탭하면 모바일 웹이 열리고, 미로그인 상태라면 로그인 후 원래 `tag`를 복원한다.
3. 프론트가 `POST /check-ins`를 호출한다.
4. 백엔드는 태그 유효성, 매장 활성 상태, 중복 체크인을 검증하고 방문 세션을 만든다.
5. 고객은 룩북을 확인하고 `PRIVATE` 또는 `STAFF_ASSISTED` 쇼핑을 선택한다.
6. `STAFF_ASSISTED`는 정보 공유 동의가 있어야 직원 요청을 생성한다.
7. 백엔드는 AI 가이드 생성을 시작하고 직원 대시보드에 방문 이벤트를 보낸다.
8. 직원 한 명이 요청을 수락하면 고객 화면과 직원 화면 양쪽에 배정 결과를 보낸다.
...
직원 응답은 이름을 `김**`처럼 마스킹하고, 주소·연락처·로그인 정보는 절대 포함하지 않는다.
...
- `customer_id`, `store_id`를 프론트가 임의로 보내게 하지 않고 인증 토큰과 NFC 태그에서 각각 결정한다.
- 같은 고객·매장에 종료되지 않은 체크인이 있으면 새 레코드를 만들지 않고 `409 ACTIVE_CHECKIN_EXISTS`와 기존 `checkin_id`를 반환한다.
- NFC 토큰은 예측 가능한 매장 ID 원문이 아닌 충분히 긴 임의 토큰으로 만든다. 분실·복제 시 비활성화 가능해야 한다.
...
- NFC URL에 고객 ID나 개인정보를 포함하지 않는다.
...
- 보존 기간은 팀 정책으로 확정하되, 동의·접근·추천 생성 이력은 감사 가능한 형태로 관리한다.
...
- 유효 NFC → 로그인 → 체크인 → 직원 응대 → 동의 → 목적 입력 → 직원 알림 → 직원 수락 → 고객 배정 화면
- 유효 NFC → 체크인 → 프라이빗 쇼핑 → 룩북 조회 → 직원에게 개인정보 미노출
...
7. **NFC 태그:** 시연 매장 ID, 실제 기록할 프론트 URL, 태그 토큰 발급 방식 확정
