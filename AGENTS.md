...
- 현재 담당 범위: 백엔드를 계약 기준으로 하는 백엔드·프론트엔드·AI 단일 저장소 통합
...
- API와 데이터 계약은 `backend/` 구현과 백엔드 명세를 기준으로 한다.
- 각 역할 폴더 안에 별도 `.git`을 만들지 않는다. 루트 Git 저장소 하나만 사용한다.
- 프론트와 AI의 mock 데이터는 화면·프롬프트 참고용이며 운영 원본으로 사용하지 않는다.
...
- 기능 구현을 완료하면 해당 내용 요약해 커밋 메시지를 추천한다.
- 각 기능 구현 완료 시 주석으로 규칙을 만든다. ex. [Backend-01-'기능명']
- 기능 구현과 관련 테스트가 완료되면 해당 변경만 커밋하고 `origin/main`으로 push한 뒤 커밋 해시를 보고한다.
